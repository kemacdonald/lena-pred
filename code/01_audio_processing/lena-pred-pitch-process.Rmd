...
The left panel shows the polynomial shape for the center of each cluster generated by the kmeans clustering. We can check whether we can reconstruct the original pitch contour by plotting the 2nd order polynomial for each 100 ms time bin (bottom row) alongside the interpolated pitch contour (top row). The number in each time bin represents the cluster assignment for that pitch shape based on the kmeans step. 

```{r plot reconstructed pitch contour, fig.width=10}
shapes_plot <- plot_cluster_shapes(d_final$centers)
...
cowplot::plot_grid(shapes_plot, recontruct_plot, ncol = 1, scale = c(0.7, 1), rel_heights = c(1, 1), rel_widths = c(0.5, 0.8))
```
