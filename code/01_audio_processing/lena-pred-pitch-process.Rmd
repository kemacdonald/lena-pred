...
min_prop_voiced <- 0.2 # min propotion of voiced samples (need to ask AW about this number)

lowest_pitch <- 75     # clips extreme pitch estimates
highest_pitch <- 1000  # clips extreme pitch estimates 
time_bin_width <- 100  # in ms

frac_points_loess <- 0.15  # what percentage point for fitting loess interpoloation curves
...
```{r}
d <- files_to_analyze[12] %>% map_df(get_pitch_contour, lowest_pitch, highest_pitch)
```

TODO: figure out why we aren't getting good pitch estimates from the ManyBabies segements. 
...
d %>% 
  ...
  distinct(seg_id, prop_voiced) -> blah
```
...
d %>% 
  ...
  filter(!is.na(pitch_interpolated)) -> d
```

## Log transform and Z-score

```{r}
d %>% mutate(log_pitch = log(pitch_interpolated), z_log_pitch = scale(log_pitch)) -> d
```
