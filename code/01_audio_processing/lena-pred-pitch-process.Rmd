...
seg_idx <- 3 # control which segment to inspect
...
segmented <- d_by_bin %>% 
  ...
  geom_line(size = 1) +
  guides(color = F) +
  facet_wrap(~time_bin_num, scales = "free_x", ncol = 10) +
  theme(legend.position = 'top') +
  ggthemes::scale_color_ptol() 
...
# one-hot encode the next cluster integers
d_lstm$next_cluster_one_hot <- one_hot_encode(d_lstm$next_cluster, cluster_dict)
```
