...
Plot the corresponding polynomial shape for the center of each cluster. These are the pitch shapes that the DNN will try to learn.

```{r}
plot_cluster_shapes(d_final$centers)
```
...
```{r plot reconstructed pitch contour, out.width="95%"}
...
plot_reconstructed_pitch(seg_id_to_plot, df_raw = d_interp, df_preds = d_by_bin) 
```
