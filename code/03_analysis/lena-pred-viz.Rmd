...
d <- d %>% mutate(speaker_id = get_speaker_id_mb(seg_id), duration_ms = as.numeric(duration_ms)) 

# temp fix
d <- d %>% mutate(is_target_cluster = ifelse(str_extract(cluster_shape, "\\d+") == target_cluster, TRUE, FALSE))
```
...
```{r}
seg_ms %>% 
  filter(duration_ms <= 20000, n_qshapes == 12) %>% 
