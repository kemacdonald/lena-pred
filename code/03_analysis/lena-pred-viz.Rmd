...
Extract preds from different runs of experiment

```{r}
d <- flatten(d_list) %>% map2_df(.y = names(.), .f = extract_preds)
...
First we aggregate the results within each run of the experiment.
...
ss_ms <- d %>% 
  ...
  group_by(seg_id, speech_register, n_qshapes, speaker_id, duration_ms, 
           exp_run_id) %>% 
  summarise(m_prob_exp = mean(prob_mass),
            m_acc_exp = mean(correct_pred))
```

Then we aggregate across expriment runs. 

```{r}
mb_ms <- ss_ms %>% 
  group_by(seg_id, speech_register, n_qshapes, speaker_id, duration_ms) %>% 
  summarise(m_prob = mean(m_prob_exp),
            m_acc = mean(m_acc_exp))
```
...
```{r}
plot_model_output(mb_ms, col_name = m_acc, y_axis_lab = "Mean Accuracy")
```
...
d_within_utt <- d %>% 
  filter(is_target_cluster == TRUE, !(seg_id %in% too_short_list)) %>% 
